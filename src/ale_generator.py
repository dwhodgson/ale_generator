...
from math import exp, pi, atan, sqrt, tan
...
map_diff_c = ale_funcs.map_diff_py
map_diff_c.argtypes = [ctypes.c_double, ctypes.c_double, ctypes.POINTER(ctypes.c_double),
                       ctypes.POINTER(ctypes.c_double), ctypes.c_int]
map_diff_c.restype = ctypes.c_double

gaps_c = ale_funcs.gaps
gaps_c.argtypes = [ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double),
                   ctypes.c_int]
...
def angle_map(c: float, theta: float) -> float:
    """Computes the image of an angle after a slit is added at the point 1.

    Returns the argument of f(e^i`theta`) where f is the map that adds a slit of capacity `c` at the point 1.
    ...
    c : float
        The logarithmic capacity of the slit.
    theta : float
        The angle of which to compute image.

    Returns
    -------
    float
        The argument of the image of e^i`theta` under the slit map.

    Raises
    ------
    ValueError
        If the image of the point e^i`theta` would be on the added slit.

    """
    angle = tan(theta / 2)
    d = length(c)
    d = d / (d + 2)
    angle = angle * sqrt(1 - d ** 2)
    if angle < -d:
        return 2 * atan(-sqrt(angle ** 2 - d ** 2))
    elif angle > d:
        return 2 * atan(sqrt(angle ** 2 - d ** 2))
    raise ValueError('This angle will be sent to a point on the new slit.')
...
def generation(m: int, theta: float, caps: List[float], angles: List[float]) -> int:
    ...
    for n in range(m, -1, -1):
        ...
        if angle + b > pi and theta <= angle + b - 2 * pi:
            return n
        # Compute image of theta under nth slit map
        theta = angle_map(c, theta - angle) + angle
        # Return to range [-pi,pi]
        while theta > pi:
            theta = theta - 2 * pi
        while theta < -pi:
            theta = theta + 2 * pi
...
class LaplacianModel:
    ...
    def compute_gaps(self, save: bool = False, plot: bool = False) -> None:
        ...
        if len(self.thetas) == 0 or len(self.caps) == 0:
            raise AttributeError('Angles or capacities (or both) have not been generated for this model')
        DoubleArray = ctypes.c_double * self.n
        IntArray = ctypes.c_int * self.n
        cap_list = DoubleArray(*self.caps)
        theta_list = DoubleArray(*self.thetas)
        gap_list = IntArray(*[0 for _ in range(self.n)])
        gaps_c(gap_list,cap_list,theta_list,self.n)
        self.gaps = gap_list
        if save:
            self.save_gaps()
        if plot:
...
class ALE(LaplacianModel):
    ...
    def sample(self) -> float:
        ...
        # Choose random bin according to Simpson weights
        total = sum(simpsons)
    ...
    def generate_angles_and_capacities(self) -> None:
        ...
        for _ in range(self.n - 1):
            ...
            z = cmath.exp(self.sigma + new_angle * 1j)
            DoubleArray = ctypes.c_double * len(self.caps)
            cap_list = DoubleArray(*self.caps)
            theta_list = DoubleArray(*self.thetas)
            diff = map_diff_c(z.real, z.imag, cap_list, theta_list, len(self.caps))
...
class HL(LaplacianModel):
    ...
    equivalent to this model with `alpha`=0. See [1]_ for mathematical explanation and justification of this model.
    ...
    References
    ----------
    .. [1] MB. Hastings, LS. Levitov, "Laplacian Growth as One-Dimensional Turbulence", Physica D, vol. 116, pp. 244-252,
    1998.
